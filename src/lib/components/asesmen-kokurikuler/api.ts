export async function downloadTemplate(
	kokurikulerId: string | number,
	kelasId: string | number
): Promise<boolean> {
	try {
		const formData = new FormData();
		formData.append('kokurikulerId', String(kokurikulerId));
		formData.append('kelasId', String(kelasId));

		const response = await fetch('/api/asesmen-kokurikuler/download-template', {
			method: 'POST',
			body: formData
		});
		...
		let filename = `template-nilai-kokurikuler-${new Date().getTime()}.xlsx`;
...
export async function importNilai(
	...
	try {
		...
		const response = await fetch('/api/asesmen-kokurikuler/import', {
			method: 'POST',
			body: formData
		});
